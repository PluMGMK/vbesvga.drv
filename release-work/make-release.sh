...
MODETEST_COMMIT=`git log -1 --pretty=format:"%h" ../MODETEST.ASM`
git status --short ../MODETEST.ASM | grep "^ M" && MODETEST_COMMIT="$MODETEST_COMMIT(modified)"
SETUP_COMMIT=`git log -1 --pretty=format:"%h" ../SETUP.ASM`
git status --short ../SETUP.ASM | grep "^ M" && SETUP_COMMIT="$SETUP_COMMIT(modified)"
...
jwasm -DMODETEST_COMMIT=\'$MODETEST_COMMIT\' -bin ../MODETEST.ASM
mv -v MODETEST.BIN MODETEST.COM
jwasm -DSETUP_COMMIT=\'$SETUP_COMMIT\' -bin ../SETUP.ASM
...
sha1sum -b VBESVGA.DRV VDDVBE.386 VBEVMDIB.3GR AUXSTACK.COM AUXCHECK.COM VIDMODES.COM MODETEST.COM SETUP.EXE OEMSETUP.INF > VBESVGA.SHA

# Zip it all up
zip -u -9 vbesvga-release.zip VBESVGA.DRV VDDVBE.386 VBEVMDIB.3GR AUXSTACK.COM AUXCHECK.COM VIDMODES.COM MODETEST.COM SETUP.EXE OEMSETUP.INF VBESVGA.SHA
mcopy -v -s -i vbesvga.img VBESVGA.DRV VDDVBE.386 VBEVMDIB.3GR AUXSTACK.COM AUXCHECK.COM VIDMODES.COM MODETEST.COM SETUP.EXE OEMSETUP.INF VBESVGA.SHA ::/
